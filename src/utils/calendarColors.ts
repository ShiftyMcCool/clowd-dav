...
// Note: Calendar colors are now stored on the CalDAV server, not in localStorage
...
/**
 * Assigns default colors to calendars that don't have server-provided colors
 * Server colors take precedence over defaults
 */
export function assignDefaultColorsIfMissing<
  T extends { url: string; color?: string }
>(calendars: T[]): T[] {
  return calendars.map((calendar, index) => ({
    ...calendar,
    color: calendar.color || DEFAULT_COLORS[index % DEFAULT_COLORS.length],
